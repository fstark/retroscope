...
uint32_t file_t::allocation_offset(uint32_t offset) const
{
    ...
    for (const auto &extent : extents_)
    {
        uint32_t extent_size = extent.count * block_size_;
        if (offset < current_offset + extent_size)
        {
            uint32_t local_offset = offset - current_offset;
            return extent.start * block_size_ + local_offset;
        }
...
void partition_t::readMasterDirectoryBlock()
{
    ...
    std::cout << std::format("Volume name: {}\n", mdb.getVolumeName());
    ...
    auto* root_folder = new Folder(mdb.getVolumeName());
    ...
    // Clean up - in a real application you'd want proper memory management
    // For this demo, we'll leave the cleanup out for simplicity
}

void partition_t::readCatalogHeader(uint64_t /* catalogExtendStartBlock */)
{
    // Implementation moved to readMasterDirectoryBlock()
}

void partition_t::readCatalogRoot(uint16_t /* rootNode */)
{
    // Implementation integrated into readMasterDirectoryBlock()
}
