...
class HFSPartition
{
	...
	struct extent_t { uint16_t start; uint16_t count; };
	...
	class file_t
	{
		...
		public:
			...
			uint16_t to_absolute_block( uint16_t block ) const
			{
				...
				{
					if (block < ext.count) return ext.start + block;
					block -= ext.count;
				}
				return 0xffff;
			}
...
//	type Folder: [ParentID, Name, File #] (to check)
	...
	HFSPartition(std::ifstream &file, uint64_t partitionStart, uint64_t partitionSize)
		: file_(file)
		, partitionStart_(partitionStart)
...
// HFS Master Directory Block (Volume Header) - corrected layout
...
void HFSPartition::readCatalogHeader( uint64_t catalogStartBlock )
{
	...
	if (nodeDesc->kind != 1) { // 1 = header node
...
void processAPM(std::ifstream& file) {
    ...
    for (uint32_t i = 1; i <= mapBlocks; i++) {
        ...
        if (be16toh_custom(entry->pmSig) != 0x504D) continue;
